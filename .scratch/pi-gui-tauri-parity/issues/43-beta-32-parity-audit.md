...
- [ ] Every included beta.32 flow row has exactly one owning ticket and one passing automated or documented acceptance check.
- [ ] All JavaScript, Rust, process-contract, real-desktop, and packaged-app quality commands pass from a clean checkout.
- [ ] A packaged smoke covers real auth, workspace opening, thread execution, restart, native picker, dynamic extension, and terminal.
- [ ] Audit confirms exclusions: update flow, website/video/Homebrew, signing/notarization, Linux/Windows, standalone computer-use, and pi-gui app-state migration.
