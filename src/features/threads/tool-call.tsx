...
export function ToolCall({ tool, onOpenFile, defaultOpen = false }: { tool: ThreadMessage; onOpenFile?: (path: string) => void; defaultOpen?: boolean }) {
  ...
  useEffect(() => { if (progressive) updateDisclosure({ type: "progressive" }); }, [progressive]);
  return <details className={`group/tool pb-tool-call pb-tool-call-${presentation.status}`} data-testid="tool-call" open={disclosure.open || undefined}>
    <summary ref={summaryRef} className="pb-button-interaction pb-tool-summary" data-effects="default" onClick={(event) => { event.preventDefault(); updateDisclosure({ type: "user-toggle" }); }}>
