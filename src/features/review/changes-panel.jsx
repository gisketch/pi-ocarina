...
export function ChangesPanel({ workspaceId, open, selectedPath, onClose }) {
  ...
  useEffect(() => { void invoke("app_state_snapshot").then(({ state }) => setWidth(state.preferences.reviewer_width || 560)); }, []);
  const resize = (/** @type {number} */ next) => { const bounded = Math.max(320, Math.min(1200, next)); setWidth(bounded); void invoke("set_panel_layout", { reviewerWidth: bounded }); };
