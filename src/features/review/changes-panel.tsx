...
export function ChangesPanel({ workspaceId, open, selectedPath, onClose }: { workspaceId: string; open: boolean; selectedPath?: string; onClose: () => void }) {
  ...
  useEffect(() => { void invoke<AppStateSnapshot>("app_state_snapshot").then(({ state }) => setWidth(state.preferences.reviewer_width || 560)); }, []);
  const resize = (next: number) => { const bounded = Math.max(320, Math.min(1200, next)); setWidth(bounded); void invoke("set_panel_layout", { reviewerWidth: bounded }); };
  useEffect(() => { if (!open) return; setError(""); void invoke<ChangedFile[]>("repository_changes", { workspaceId }).then((items) => { setFiles(items); setSelected(selectedPath && items.some((item) => item.path === selectedPath) ? selectedPath : items[0]?.path ?? ""); }).catch((cause) => setError(String(cause))); }, [open, selectedPath, workspaceId]);
  useEffect(() => { if (!selected) { setDiff(null); return; } setDiff(null); void invoke<Diff>("file_diff", { workspaceId, path: selected }).then(setDiff).catch((cause) => setError(String(cause))); }, [selected, workspaceId]);
  useEffect(() => { if (!open || mode !== "files") return; void invoke<WorkspaceFile[]>("workspace_files", { workspaceId }).then(setWorkspaceFiles).catch((cause) => setError(String(cause))); }, [mode, open, workspaceId]);
  const openFile = (path: string) => void invoke<OpenFile>("read_workspace_file", { workspaceId, path }).then(setFile).catch((cause) => setError(String(cause)));
