...
export function TerminalPanel({ workspaceId }) {
  const [tabs, setTabs] = useState(/** @type {Tab[]} */ ([]));
  ...
  useEffect(() => {
    ...
    const listeners = Promise.all([
      ...
      listen("terminal://closed", ({ payload }) => terminals.current.get(payload.terminalId)?.terminal.write("\r\n[process exited]\r\n")),
    ]);
  ...
  async function addTab() {
    try {
      ...
      const id = await invoke("open_terminal", { workspaceId, cols: 80, rows: 24 });
  ...
  function attachTerminal(id, node) {
