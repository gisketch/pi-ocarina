...
import { useState, type ComponentProps, type PointerEvent, type Ref } from "react";
...
function Button({ className, variant = "default", size = "default", asChild = false, effects = "default", children, onPointerEnter, ref, ...props }: ButtonProps) {
  ...
  return <Comp ref={fieldRef} data-slot="button" data-variant={variant} data-size={size} data-effects={effects} onPointerEnter={(event) => { if (effects === "default") setRevealing(true); onPointerEnter?.(event as PointerEvent<HTMLButtonElement>); }} className={cn(buttonVariants({ variant, size }), className)} {...props}>{!asChild && effects === "default" && revealing ? <StaggeredButtonText onComplete={() => setRevealing(false)}>{children}</StaggeredButtonText> : children}</Comp>;
}
