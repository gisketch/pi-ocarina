...
const exclusionBlock = flows.match(/## Explicit Exclusions\n\n([\s\S]*?)\n\n##/)?.[1] ?? "";
const exclusions = exclusionBlock.split("\n").filter((line) => line.startsWith("- "));
if (exclusions.length !== 5) errors.push(`expected 5 exclusion groups, found ${exclusions.length}`);
const excludedRows = rows.filter((row) => (row[3] ?? "").startsWith("Excluded —")).length;
